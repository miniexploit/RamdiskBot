...
class ramdiskMaker:
	def __init__(self, identifier, version, callback=None):
		self.callback = callback if callback else print
		async_run(self.callback(f"init: {identifier}, {version}, {callback}"))
	...
	def isOutExists(self):
		try:
			return utils.read_key(self.tempdir)
		except:
			return None
	...
	def patchRamdisk(self, path):
		...
		ret = subprocess.run(["img4","-i",f"{self.tempdir}/ramdisk.dmg","-o",f"ramdisk_{self.tempdir}.im4p","-A","-T","rdsk"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		if ret.returncode != 0:
			raise ProgressError(f"Repacking ramdisk failed with return code: {ret.returncode}\nstdout={ret.stdout}\nstderr={ret.stderr}")
		async_run(self.callback("patch done"))
		return f"ramdisk_{self.tempdir}.im4p"

	def uploadRamdisk(self, path):
		async_run(self.callback("Uploading ramdisk"))
		# Find some way to upload the ramdisk yourself
		# and save the URL to url.json
		# and return the URL here
		return url

	def cleanUp(self):
		async_run(self.callback("Cleaning up"))
		if os.path.isfile(f"ramdisk_{self.tempdir}.im4p"):
			os.remove(f"ramdisk_{self.tempdir}.im4p")
		if os.path.exists(self.tempdir):
			shutil.rmtree(self.tempdir)
